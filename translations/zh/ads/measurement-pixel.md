...
| Event name | 是 | 一个[受支持的 event name](https://developers.openai.com/ads/supported-events)，例如 `order_created`，或 `"custom"`。 |
| Event data | 是 | 一个对象，其 `type` 与 event 的 [data shape](https://developers.openai.com/ads/supported-events#event-data-shapes) 匹配。 |
