...
**使用 PLANS.md 模板：** 对更高级的工作流，你可以配置 Codex 遵循 `PLANS.md` 或执行计划模板，用于较长或多步骤工作。更多细节见 [执行计划指南](/cookbook/articles/codex_exec_plans)。
...
- **云线程** 在隔离的 [environment](/codex/cloud/environments) 中运行。Codex 会克隆你的仓库并检出它正在处理的分支。当你想并行运行工作或从另一台设备委派任务时，云线程很有用。要让云线程使用你的仓库，请先把代码推送到 GitHub。你也可以[从本地机器委派任务](/codex/ide/cloud-tasks)，其中会包含当前工作状态。
...
[配置和状态位置](/codex/config-advanced#config-and-state-locations)。
...
extension](/codex/ide/slash-commands) 或 [CLI](/codex/cli/slash-commands#set-or-view-a-task-goal-with-goal) 中使用 `/goal` 启动目标模式。
...
快速模式会把受支持模型的速度提高 1.5 倍，并以高于
标准模式的速率消耗 credits。它当前支持 GPT-5.5 和 GPT-5.4，
GPT-5.5 按标准速率的 2.5 倍消耗 credits，GPT-5.4 按 2 倍消耗。
...
使用 ChatGPT 登录时，快速模式可在 Codex IDE extension、Codex CLI
...
加速受支持模型的快速模式，Codex-Spark 是自己的模型选择，
并有自己的使用限制。
...
2.  确认设置
    ...
    - **Deep scan（深度扫描）：** Off
    - **Scan area（扫描区域）：** `Entire codebase`
    - **Threat model scoping guidance（威胁模型范围指导）：** 留空，除非你已经知道某个
      具体攻击向量或应用区域值得优先处理。
...
> 用于账号变更的公开 API。接受 JSON 请求和文件上传。使用内部认证服务进行身份检查，并通过内部服务写入账单变更。重点审查认证检查、上传解析以及服务间信任边界。
...
- **沙盒模式**：当 Codex 执行模型生成的命令时，它在技术上可以做什么（例如，可以在哪里写入，以及是否能访问网络）。
- **审批策略**：Codex 何时必须在执行动作前询问你（例如，离开沙盒、使用网络，或运行不在受信任集合中的命令）。
...
**Ubuntu AppArmor 说明：** 在 Ubuntu 25.04 上，从
...
配置文件、认证流程、模型选择和配置参考资料。

### 配置参考

来源：[Configuration Reference](/codex/config-reference.md)

将本页作为 Codex 配置文件的可搜索参考。概念指引和示例请从 [配置基础](/codex/config-basic) 和 [高级配置](/codex/config-advanced) 开始。
...
关于项目指导、可复用能力、自定义斜杠命令、subagent 工作流和集成的背景，请参阅 [自定义](/codex/concepts/customization)。配置键见 [配置参考](/codex/config-reference)。

#### 配置档案

Profiles 让你保存命名配置层，并从
CLI 在它们之间切换。当你传递 `--profile profile-name` 时，Codex 会加载
`~/.codex/config.toml`，然后叠加 `~/.codex/profile-name.config.toml`。
Profile 名称可以包含字母、数字、连字符和下划线。

为每个 profile 创建单独的 TOML 文件。在
profile 文件中使用顶层配置键；不要把它们嵌套在 `[profiles.profile-name]` 下。
...
因为 profile 文件是位于基础用户配置之上、project 和 CLI 配置之下的一层，
它只需要包含不同于基础
配置的值。Profile 文件也可以覆盖 `model_catalog_json`；当两个文件都设置它时，Codex 使用
profile 值。

在 Codex 0.134.0 及更高版本中，`--profile` 不再从 `config.toml` 读取 `[profiles.profile-name]`，
且不再支持顶层 `profile = "profile-name"` selector。请将旧版 profile settings 移入
...
- `--config` 值会作为 TOML 解析。拿不准时，请给值加引号，避免 shell 按空格拆分。
- 如果值无法解析为 TOML，Codex 会把它视为字符串。
...
- `config.toml`（你的本地配置）
- `auth.json`（如果你使用基于文件的凭据存储）或你的 OS keychain/keyring
- `history.jsonl`（如果启用历史持久化）
- 其他按用户保存的状态，例如日志和缓存

认证细节（包括凭据存储模式）见 [Authentication](/codex/auth)。完整配置键列表见 [Configuration Reference](/codex/config-reference)。

关于提交到仓库或系统路径的共享默认值、rules 和 skills，见 [Team Config](/codex/enterprise/admin-setup#team-config)。
...
除了你的用户配置，Codex 还会读取 repo 内 `.codex/config.toml` 文件中的项目作用域覆盖。Codex 会从项目根目录走到当前工作目录，并加载它找到的每个 `.codex/config.toml`。如果多个文件定义同一个键，离你的工作目录最近的文件获胜。

出于安全考虑，Codex 只会在项目受信任时加载项目作用域配置文件。如果项目不受信任，Codex 会忽略项目 `.codex/` layers，包括 `.codex/config.toml`、project-local hooks 和 project-local rules。用户层和系统层仍保持独立并继续加载。
...
project-local `.codex/config.toml` 中的以下键，并在看到它们时打印 startup
...
当前事件列表、输入字段、输出行为和限制见
[Hooks](/codex/hooks)。

#### 智能体角色（`config.toml` 中的 `[agents]`）

关于 subagent 角色配置（`config.toml` 中的 `[agents]`），见 [Subagents](/codex/subagents)。

#### 项目根目录检测

Codex 会通过从工作目录向上遍历，直到到达项目根目录，来发现项目配置（例如 `.codex/` layers 和 `AGENTS.md`）。

默认情况下，Codex 会把包含 `.git` 的目录视为项目根目录。要自定义此行为，请在 `config.toml` 中设置 `project_root_markers`：
...
将 `project_root_markers = []` 设为空，即可跳过搜索父目录，并把当前工作目录视为项目根目录。

#### 自定义模型 providers

模型 provider 定义 Codex 如何连接到模型（base URL、wire API、authentication 和可选 HTTP headers）。自定义 providers 不能复用保留的内置 provider IDs：`openai`、`ollama` 和 `lmstudio`。
...
当 provider 需要 Codex 从外部 credential helper 获取 bearer tokens 时，使用命令驱动的 authentication：
...
auth command 不接收 `stdin`，并且必须把 token 打印到 stdout。Codex 会修剪前后空白，将空 token 视为错误，并按 `refresh_interval_ms` 主动刷新；设置 `refresh_interval_ms = 0` 则只在 authentication retry 后刷新。不要将 `[model_providers..auth]` 与 `env_key`、`experimental_bearer_token` 或 `requires_openai_auth` 组合使用。

#### Amazon Bedrock provider

Codex 包含内置 `amazon-bedrock` model provider。直接把它设为
`model_provider`；不同于 custom providers，该内置 provider 只支持
嵌套 AWS profile 和 region overrides。
...
完整 setup flow、authentication options、supported models 和功能可用性见 [Use Codex with Amazon Bedrock](/codex/amazon-bedrock)。

#### OSS 模式（local providers）
...
对于 CLI，当没有有效 session 时，使用 ChatGPT 登录是默认认证路径。
...
当你使用 ChatGPT 或 API key 登录 Codex app、CLI 或 IDE Extension 时，Codex 会缓存你的登录详情，并在下次启动 CLI 或 extension 时复用它们。CLI 和 extension 共享同一份缓存登录详情。如果你从其中任一处注销，下次启动 CLI 或 extension 时都需要重新登录。

Codex 会将登录详情本地缓存在 `~/.codex/auth.json` 明文文件中，或你的 OS-specific credential store 中。

对于使用 ChatGPT 登录的 sessions，Codex 会在使用期间、tokens 过期前自动刷新，因此活动 sessions 通常会继续，无需再次浏览器登录。
...
- `file` 将凭据存储在 `CODEX_HOME` 下的 `auth.json` 中（默认 `~/.codex`）。
- `keyring` 将凭据存储在你的操作系统凭据存储中。
- `auto` 在可用时使用 OS credential store，否则回退到 `auth.json`。

如果你使用基于文件的存储，请像对待密码一样对待 `~/.codex/auth.json`：
它包含 access tokens。不要提交它、粘贴到 tickets，或在
chat 中分享。
...
成熟度列使用 Experimental、Beta
和 Stable 等功能成熟度标签。请参阅 [功能成熟度](/codex/feature-maturity)，了解如何
解读这些标签。
...
| `codex://settings`                           | 设置。                                                          |
| `codex://settings/connections/`              | 计算机、device 或 SSH 连接设置。                                |
| `codex://settings/connections/ssh/add?name=` | 将你的 SSH config 中的 host 添加到 Codex。                       |
| `codex://skills`                             | Skills 页面。                                                    |
| `codex://automations`                        | Automations，并打开创建流程。                                    |
| `codex://plugins/install/?marketplace=`      | 来自已知 marketplace 的 plugin 安装流程。                        |
| `codex://plugins/`                           | plugin 详情页。                                                  |
| `codex://plugins/?marketplacePath=`          | 来自本地 marketplace 的本地 plugin 详情页。                      |
...
| --------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prompt=`       | 否       | 设置初始 composer text。                                                                                                                                        |
| `path=`         | 否       | 在 local workspace 中打开新 thread。`path` 必须是指向本地目录的 absolute path。有效时，Codex 会使用该目录作为 active workspace。                               |
| `originUrl=`    | 否       | 按 Git remote URL 匹配你当前 workspace roots 之一。如果同时存在 `path`，Codex 会先解析 `path`。                                                                 |
...
| `codex://settings/computer-use/google-chrome` | Computer Use 的 Google Chrome 设置。                                                         |
...
| `codex://settings/connections/ssh/add?name=`  | 将命名 host alias 添加为 Codex-managed connection，然后打开 SSH 连接设置。                   |
...
不受支持的 `codex://settings/...` paths 会打开主设置页。
...
| Deep link                               | 打开                                            |
| --------------------------------------- | ----------------------------------------------- |
| `codex://plugins/install/?marketplace=` | plugin 详情或安装流程。                         |

| Query parameter | 是否必需 | 作用                                                                            |
| --------------- | -------- | ------------------------------------------------------------------------------- |
| `marketplace=`  | 是       | 标识 marketplace。对于 OpenAI-curated plugin，请使用 `openai-curated`。         |

安装链接只接受 `marketplace` query parameter。如果 Codex 找不到请求的 marketplace 或 plugin，它会改为打开 Plugins 页面。
...
| Query parameter | 是否必需 | 作用                                                                                                                                            |
| --------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `hostId=`       | 否       | 标识拥有 plugin context 的 Codex host，例如 `local` 或你的某个已配置 remote connections。Codex 会提供这些 IDs。                                |
| `source=manage` | 否       | 保留 app 的 plugin-management entry point。它不是 admin-only。                                                                                 |
...
| Query parameter    | 是否必需 | 作用                                                                                                       |
| ------------------ | -------- | ---------------------------------------------------------------------------------------------------------- |
| `marketplacePath=` | 是       | 指向本地 `marketplace.json` 的绝对路径，例如 `/Users/alex/.agents/plugins/marketplace.json`。              |
...
| --------------- | -------- | ------------------------------------------------- |
| `name=`         | 是       | 设置 pet name。                                   |
| `imageUrl=`     | 是       | 设置 pet image URL。`imageUrl` 必须为 HTTPS。     |
| `description=`  | 否       | 设置可选 pet description。                        |
...
单独的后台 thread，请明确提出该请求，例如：`请在此项目的 worktree 中创建一个单独的后台 thread 来更新测试。`
...
使用设置面板调整 Codex app 的行为、它如何打开文件，
以及它如何连接工具。从 app menu 打开 [**Settings**](codex://settings)，或
按 Cmd+,。

#### 通用

选择文件在哪里打开、threads 中显示多少命令输出，以及
terminal tabs 默认在哪里打开。你还可以要求 Cmd+Enter
用于多行 prompts，或防止 thread 运行期间进入 sleep。

#### 个人资料

使用 **Profile** 查看活动洞察、累计 tokens、峰值 tokens、
连续使用记录、最长任务和 token 活动。你还可以更新 profile
详情，例如头像、显示名称和用户名，并保存带有使用亮点的 profile
card。面向个人用户的
ChatGPT plans 支持分享 profile cards。

符合条件的用户也可以从 profile menu 发送 Codex 邀请。个人 plan 符合条件时选择
...
打开 **Keyboard Shortcuts** 以查看 commands、更改 bindings，或将自定义
shortcuts 重置为默认值。使用搜索字段按 command
name 查找 shortcuts，或切换到按键搜索并按下组合键，以找到
使用它的 command。

#### 通知

选择 turn 完成通知何时出现，以及 app 是否应提示获取
通知权限。

#### 智能体配置

app 中的 Codex agents 会继承与 IDE 和 CLI extension 相同的配置。
使用 app 内控件设置常见选项，或编辑 `config.toml` 进行高级
配置。更多详情请参阅 [Codex security](/codex/agent-approvals-security) 和
[config basics](/codex/config-basic)。

#### 外观

在 **Settings** 中，你可以通过选择基础主题、
调整强调色、背景色和前景色，以及更改 UI 和代码
字体来改变 Codex app 外观。你还可以与朋友分享自定义主题。

#### Codex pets

Codex pets 是 app 的可选动画陪伴。在 **Settings** 中，
进入 **Appearance** 并选择 **Pets**，以选择内置 pet 或
从你的本地 Codex home 刷新自定义 pets。在
composer 中输入 `/pet`，在 **Settings > Appearance** 中使用 **Wake Pet** 或 **Tuck Away Pet**，或
按 Cmd+K 或 Ctrl+K 并运行相同 commands，以
切换浮动 overlay。

    该 overlay 会在你使用其他 app 时让活动中的 Codex 工作保持可见。
    它显示活动 thread，反映 Codex 是正在运行、等待输入还是准备好供审查，
    并把该状态与简短进度提示配对，让你不用重新打开 thread 也能扫一眼发生了什么变化。
...
从 command menu 重新加载 skills。按 Cmd+K 或 Ctrl+K，
...
通过 MCP (Model Context Protocol) 连接外部工具。启用推荐 servers 或
添加你自己的 server。如果 server 需要 OAuth，app 会启动认证流程。这些 settings
...
[Codex Chrome extension](/codex/app/chrome-extension)，并管理允许和
阻止的网站。除非你已允许，否则 Codex 会在使用某个 website 前询问。
移除被阻止的网站会让 Codex 在 browser 中使用它之前再次询问。

在 **Developer mode** 下，打开 **Enable full CDP access**，让 Codex 使用
Chrome DevTools Protocol 进行性能分析和更深入的 browser
调试。如果你的组织已禁用 full CDP access，你无法在本地启用
...
设置完成后，请检查 Computer Use settings，以查看 desktop-app access 和相关
偏好。在 macOS 上，可通过更新 macOS Privacy & Security settings 中的 Screen
Recording 或 Accessibility permissions 来撤销系统级访问。
...
使用感知上下文的建议展示你在启动或返回 Codex 时可能想恢复的 follow-ups 和 tasks。
...
对于本地开发服务器、基于文件的预览，以及不需要
登录的公共页面，请先使用 [in-app browser](/codex/app/browser)。该
in-app browser 会将预览和验证工作保持在 Codex 内部，而不使用
你的 Chrome profile。

Codex 也可以根据 task 需要在工具之间切换：当有
专用集成可用时使用 plugins，需要已登录 browser
...
plugin setup 完成后，启动新的 Codex thread。当 task 需要已登录网站时，Codex 可以建议
...
默认情况下，Codex 会在与每个新网站交互前询问。Codex 会基于
网站 host（例如 `example.com`）发出提示。

当 Codex 要求使用某个网站时，你可以选择与
task 和你的风险承受能力匹配的选项：

- 允许当前 chat 使用该网站。
- 始终允许该 host，以便 Codex 日后无需询问即可再次使用该网站。
- 拒绝该网站。
...
#### 始终允许 browser content

如果你开启 always allow browser content，Codex 不再要求

在使用 websites 前确认。

#### 浏览器历史记录

Browser history 可能包含 sensitive telemetry、internal URLs、search terms，
...
如何用指令、skills、prompts、MCP 和外部集成塑造 Codex 行为。
...
| `SYSTEM`                                                                       | OpenAI 随 Codex 捆绑提供。                                                                                                                                                                          | 适用于广泛受众的有用 skills，例如 skill-creator 和 plan skills。每个人启动 Codex 时都可用。                                                                                                      |
...
Codex 会在你的仓库中搜索 `AGENTS.md` 文件，并遵循你包含的任何 **Review guidelines**（审查指南）。
...
3. 在任何地方运行 Codex，确认它加载了该文件：
   ...
   预期结果：Codex 在提出工作前引用 `~/.codex/AGENTS.md` 中的条目。
...
3. 从 payments 目录启动 Codex：
   ...
   预期结果：Codex 先报告全局文件，其次是仓库根目录的 `AGENTS.md`，最后是 payments override。
...
预期结果：在斜杠命令菜单中输入 `/prompts:draftpr` 会显示你的自定义命令，并带有 front matter 中的描述，同时提示 files 和 PR title 是可选项。
...
预期结果：Codex 会展开 `draftpr.md` 的内容，用你提供的参数替换 placeholders，然后将结果作为消息发送。
...
自定义是让 Codex 按你的团队工作方式工作的方式。

在 Codex 中，自定义来自几个协同工作的层：
...
**更新 `AGENTS.md`：** 从真正重要的指令开始。将反复出现的审查反馈编入规范，把指导放在最接近适用位置的目录中，并在你纠正某件事时告诉 agent 更新 `AGENTS.md`，使未来会话继承该修复。

#### 何时更新 `AGENTS.md`

- **重复错误**：如果 agent 反复犯同样的错误，请添加规则。
- **读取过多**：如果它找到了正确文件但读取了太多文档，请添加路由指导（优先读取哪些目录/文件）。
- **反复出现的 PR 反馈**：如果你多次留下相同反馈，请将其编入规范。
...
面向 CI、SDK 使用、app-server、GitHub Actions 以及相关智能体工具的自动化路径。
...
TypeScript 库提供一种从应用内部控制 Codex 的方式，相比非交互模式更全面、更灵活。
...
**`codex`**：运行 Codex session。接受与 Codex `Config` struct 匹配的配置参数。`codex` 工具接受以下 properties：
...
| `config`                | `object`  | 会覆盖 `$CODEX_HOME/config.toml` 中内容的单项配置设置。                                  |
...
| 能力                                                          | Workspace owners and admins                          | 拥有 access token 权限的成员                  | 没有 access token 权限的成员           |
| ------------------------------------------------------------- | ---------------------------------------------------- | --------------------------------------------- | -------------------------------------- |
...
Codex 让企业团队能够看到采用情况和影响，并提供安全与合规项目所需的可审计性。使用自助 dashboard 进行日常跟踪，使用 Analytics API 进行编程式报告，并使用 Compliance API 将详细 logs 导出到你的治理栈。
...
analytics dashboard 允许 ChatGPT workspace administrators 和 analytics viewers 跟踪 Codex 采用情况、使用量和 Code Review feedback。Usage data 可能最多滞后 12 小时。
...
#### Codex 如何在本地应用云端托管 requirements
...
Domain 语法、local/private destination rules、deny-over-allow 行为
和 DNS rebinding limitations，与 [Agent approvals & security](/codex/agent-approvals-security#network-isolation) 中描述的 sandbox networking 行为相同。
...
使用 `config.toml` 的 `[features]` table 中的 canonical feature keys。Codex 会规范化最终 feature set 以满足这些 pins，并拒绝对 `config.toml` 或 profile file feature settings 的冲突写入。
...
| 导入项                              | Codex 目标                              |
| ----------------------------------- | --------------------------------------- |
| 指令文件                            | [`AGENTS.md`](/codex/guides/agents-md) |
...
| 最近 30 天的聊天 sessions           | Codex threads                          |
...
- Bug reports and feature requests（bug 报告和功能请求）：[openai/codex/issues](https://github.com/openai/codex/issues)
- Discussion forum（讨论论坛）：[openai/codex/discussions](https://github.com/openai/codex/discussions)
...
安装 plugin 后，你可以直接在 prompt window 中使用它：
    ...
    直接描述任务
      ...
      说明你想要的结果，例如“总结今天以来的未读 Gmail threads”或“从 Google Drive 拉取最新发布说明”。

      当你希望 Codex 自行选择合适的已安装工具完成任务时使用这种方式。

    选择特定 plugin

      输入 @ 以显式调用该 plugin 或其捆绑的某个 skill。

      当你想明确指定 Codex 应使用哪个 plugin 或 skill 时使用这种方式。参见 Codex app 命令和
      Skills。
...
远程连接让你可以从另一台设备或另一台机器使用 Codex。使用 ChatGPT mobile app 在已连接的 Mac 或 Windows 设备上使用 Codex，从另一台受支持的 Codex App 设备继续工作，或将 Codex App 连接到 SSH host 上的 projects。

远程访问使用已连接 host 的 projects、threads、files、credentials、permissions、plugins、Computer Use、browser setup 和 local tools。
...
| Site 需求                                                     | 要向 Sites 请求什么                                                           |
| -------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| 内容驱动的网站或落地页                                        | 除非体验需要，否则不带持久 application state 的 site                          |
| 保存的记录、用户进度或游戏分数                                | D1，用于 durable structured data 的 relational database                        |
| 图片、文档、音频、视频或其他上传内容                          | R2，用于 files 的 object storage                                               |
| 带可搜索 metadata 的上传文件                                  | D1 用于 metadata，R2 用于 file contents                                        |
| 需要当前 workspace 用户身份的内部站点                         | Workspace-authenticated user identity                                          |
| 公开登录或外部身份 provider                                   | 启用 authentication 的 Sites project                                           |
...
全局 subagent settings 仍位于 [configuration](/codex/config-basic#configuration-precedence) 的 `[agents]` 下。

| 字段                             | 类型   | 是否必需 | 用途                                                       |
| -------------------------------- | ------ | :------: | ---------------------------------------------------------- |
| `agents.max_threads`             | number |    否    | 可同时打开的 agent thread 数量上限。                       |
| `agents.max_depth`               | number |    否    | 生成 agent 的嵌套深度（根 session 从 0 开始）。            |
| `agents.job_max_runtime_seconds` | number |    否    | `spawn_agents_on_csv` jobs 的每个 worker 默认超时时间。    |
...
| 字段                     | 类型     | 是否必需 | 用途                                                            |
| ------------------------ | -------- | :------: | --------------------------------------------------------------- |
| `name`                   | string   |   是     | Codex 生成或引用此 agent 时使用的 agent 名称。                  |
| `description`            | string   |   是     | 面向用户的说明，描述 Codex 应何时使用此 agent。                 |
| `developer_instructions` | string   |   是     | 定义 agent 行为的核心指令。                                     |
| `nickname_candidates`    | string[] |   否     | 供生成的 agents 使用的可选展示昵称池。                          |
...
| 最新且完全更新的 Windows 10      | Best effort     | 可以工作，但不如 Windows 11 可靠。对于 Windows 10，Codex 依赖 modern console support，包括 ConPTY。实际使用中，需要 Windows 10 version 1809 或更高版本。                             |
| 较旧的 Windows 10 builds         | Not recommended | 更可能缺少所需 console components（例如 ConPTY），也更可能在 enterprise setups 中失败。                                                                                              |
