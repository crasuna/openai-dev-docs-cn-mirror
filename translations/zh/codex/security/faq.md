...
threat model 是扫描时用于 repository 的安全上下文。它把简明项目概览与攻击面详情结合起来，例如 entry points、trust boundaries、auth assumptions 和 risky components。更多详情请参见 [Improving the threat model](https://developers.openai.com/codex/security/threat-model)。
...
可以。Codex Security 会创建初始 threat model，你可以随着架构、风险和业务上下文变化而更新它。有关编辑工作流，请参见 [Improving the threat model](https://developers.openai.com/codex/security/threat-model)。

### 使用 threat modeling 前需要配置扫描吗？

需要。Threat-model guidance 与你如何扫描以及扫描什么绑定，因此你需要先配置 repository。参见 [Codex Security setup](https://developers.openai.com/codex/security/setup)。
