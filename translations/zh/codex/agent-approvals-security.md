---
status: needs-review
sourceId: "0e4565feb798"
sourceChecksum: "0e4565feb798d5201c921e447b5c7c357a7715bde53f3d6f0a2a3ce0ea9493af"
...
本页介绍如何安全地运行 Codex，包括 sandboxing、approvals 和网络访问。如果你要查找用于扫描已连接 GitHub 仓库的 Codex Security 产品，请参阅 [Codex Security](https://developers.openai.com/codex/security)。
...
关于 Codex app、IDE extension 和 CLI 中 sandboxing 工作方式的高层说明，请参阅 [sandboxing](https://developers.openai.com/codex/concepts/sandboxing)。更广泛的企业安全概览请参阅 [Codex security white paper](https://trust.openai.com/?itemUid=382f924d-54f3-43a8-a9df-c39e6c959958&source=click)。
...
对于 Codex cloud，请参阅 [agent internet access](https://developers.openai.com/codex/cloud/internet-access)，以启用完整 internet access 或 domain allow list。
...
管理员管理的 `experimental_network` requirements 与用户 feature toggle 是分开的。它们可以在没有 `features.network_proxy` 的情况下配置并启动 sandboxed networking，但在 active sandbox 保持网络关闭时不会开启网络访问。管理员侧 `requirements.toml` 的结构请参阅 [Managed configuration](https://developers.openai.com/codex/enterprise/managed-configuration#configure-network-access-requirements)。
...
完整 reviewer lifecycle、触发条件、配置优先级和失败行为请参阅 [Auto-review](https://developers.openai.com/codex/concepts/sandboxing/auto-review)。
...
[默认 reviewer policy](https://github.com/openai/codex/blob/main/codex-rs/core/src/guardian/policy.md) 位于开源 Codex 仓库。企业可以用 managed requirements 中的 `guardian_policy_config` 替换其 tenant-specific section。也支持本地 `[auto_review].policy` 文本，但 managed requirements 优先。设置详情请参阅 [Managed configuration](https://developers.openai.com/codex/enterprise/managed-configuration#configure-automatic-review-policy)。
...
更广泛的配置工作流请参阅 [Config basics](https://developers.openai.com/codex/config-basic)、[Advanced Config](https://developers.openai.com/codex/config-advanced#approval-policies-and-sandbox-modes) 和 [Configuration Reference](https://developers.openai.com/codex/config-reference)。
...
你还可以将 presets 保存为 [profile files](https://developers.openai.com/codex/config-advanced#profiles)，然后使用 `codex --profile profile-name` 选择它们：
...
- **Windows** 在 [Windows Subsystem for Linux 2 (WSL2)](https://developers.openai.com/codex/windows#windows-subsystem-for-linux) 中运行时使用 Linux sandbox implementation。WSL1 支持到 Codex `0.114`；从 `0.115` 开始，Linux sandbox 迁移到 `bwrap`，因此不再支持 WSL1。原生运行在 Windows 上时，Codex 使用 [Windows sandbox](https://developers.openai.com/codex/windows#windows-sandbox) implementation。
...
这可确保 IDE extension 即使在 host OS 是 Windows 时，也继承 Linux sandbox semantics，用于 commands、approvals 和 filesystem access。更多信息见 [Windows setup guide](https://developers.openai.com/codex/windows)。
...
详情请参阅 [Windows setup guide](https://developers.openai.com/codex/windows#windows-sandbox)。
...
- 如果你不希望 Codex 在 `CODEX_HOME` 下保存 session transcripts，请 review local data retention settings（例如 `history.persistence` / `history.max_bytes`）。参见 [Advanced Config](https://developers.openai.com/codex/config-advanced#history-persistence) 和 [Configuration Reference](https://developers.openai.com/codex/config-reference)。
...
Enterprise admins 可以在 [Managed configuration](https://developers.openai.com/codex/enterprise/managed-configuration) 中为其 workspace 配置 Codex security settings。设置和 policy 详情见该页面。
