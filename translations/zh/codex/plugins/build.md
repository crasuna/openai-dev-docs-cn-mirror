---
status: needs-review
sourceId: "4ee6b01fbb21"
sourceChecksum: "4ee6b01fbb21c65367ea8e9a14c76030c71748f77fd65535bdcbb973f6eb5684"
sourceUrl: "https://developers.openai.com/codex/plugins/build"
...
本页面面向 plugin authors。如果你想在 Codex 中浏览、安装和使用 plugins，请参阅 [Plugins](https://developers.openai.com/codex/plugins)。如果你仍在迭代一个 repo 或一个个人工作流，请从 local skill 开始。当你希望跨团队共享该工作流、打包 app integrations 或 MCP config、封装 lifecycle hooks，或发布稳定 package 时，再构建 plugin。
...
### 构建你自己的 curated plugin list {#build-your-own-curated-plugin-list}
...
3. 将 plugin 添加到 marketplace。使用 `@plugin-creator` 生成一个，或按照[构建你自己的 curated plugin list](#build-your-own-curated-plugin-list) 将 plugin 手动接入 Codex。
...
Marketplace file 指向 plugin location，因此这些目录只是示例，而不是固定要求。Codex 会相对于 marketplace root 解析 `source.path`，而不是相对于 `.agents/plugins/` folder。请参阅 [Marketplace metadata](#marketplace-metadata) 了解文件格式。

更改 plugin 后，请更新 marketplace entry 指向的 plugin directory，并重启 Codex，以便 local install 获取新文件。

### 与你的 workspace 共享 local plugin {#share-a-local-plugin-with-your-workspace}
...
Plugin hooks 使用与 regular hooks 相同的 event schema。请参阅 [Hooks](https://developers.openai.com/codex/hooks)，了解 supported events、inputs、outputs、trust review 和 current limitations。
