---
status: needs-review
sourceId: "9aba57cd82cc"
sourceChecksum: "9aba57cd82cc8f07b0dc08a3c74c2040d7fcae1502b7238fe47e6963e46d3f57"
...
**`output` 数组通常不止一个条目！** 它可以包含 tool calls、由[推理模型](https://developers.openai.com/api/docs/guides/reasoning)生成的 reasoning tokens 相关数据，以及其他条目。不能安全地假设模型文本输出一定出现在 `output[0].content[0].text`。

为了方便使用，我们的一些[官方 SDKs](https://developers.openai.com/api/docs/libraries)会在模型响应上提供 `output_text` 属性，它会把模型的所有文本输出聚合为单个字符串。它可以作为访问模型文本输出的快捷方式。

除了纯文本，你还可以让模型以 JSON 格式返回结构化数据，此功能称为 [**Structured Outputs**](https://developers.openai.com/api/docs/guides/structured-outputs)。
...
- **[推理模型](https://developers.openai.com/api/docs/guides/reasoning)** 会生成内部思维链来分析输入 prompt，并擅长理解复杂任务和多步骤规划。与 GPT models 相比，它们通常也更慢、使用成本更高。
...
请注意，`instructions` 参数仅适用于当前 response generation request。如果你使用 `previous_response_id` 参数[管理 conversation state](https://developers.openai.com/api/docs/guides/conversation-state)，之前轮次使用的 `instructions` 不会出现在上下文中。
...
多轮对话可能由这些类型的若干 messages 组成，并包含你和模型提供的其他内容类型。进一步了解如何[管理 conversation state](https://developers.openai.com/api/docs/guides/conversation-state)。
...
OpenAI 正在弃用 API 中的可复用 prompt objects。Prompt 创建将从 2026 年 6 月 3 日开始弱化，`v1/prompts` 计划在 2026 年 11 月 30 日关闭。请查看 [deprecations 页面](https://developers.openai.com/api/docs/deprecations#2026-06-03-reusable-prompts)了解当前时间线。
...
如果你的集成已经使用 prompt ID 或版本调用保存的 prompt，请使用 [prompt object 迁移指南](https://developers.openai.com/api/docs/guides/prompting/migrate-from-prompt-object)将该 prompt 移入代码。
...
- **Instructions:** 向模型提供如何生成你想要响应的 guidance。它应该遵循哪些规则？模型应该做什么，以及绝不能做什么？此部分可以包含与你的用例相关的许多子部分，例如模型应如何[调用自定义函数](https://developers.openai.com/api/docs/guides/function-calling)。
...
<div data-content-switcher-pane data-value="prompt">
    <div class="hidden">示例 prompt</div>
...
构造 message 时，你应尽量将预计会在 API 请求中反复使用的内容放在 prompt 开头，**并且** 放在传给 [Chat Completions](https://developers.openai.com/api/docs/api-reference/chat) 或 [Responses](https://developers.openai.com/api/docs/api-reference/responses) 的 JSON request body 中最靠前的 API 参数里。这可以帮助你最大化 [prompt caching](https://developers.openai.com/api/docs/guides/prompt-caching) 带来的成本和延迟节省。

## Few-shot learning

Few-shot learning 让你通过在 prompt 中包含少量输入/输出 examples，将大型语言模型引导到新任务，而不是[微调](https://developers.openai.com/api/docs/guides/model-optimization)模型。模型会隐式“学到”这些 examples 中的模式，并将其应用于 prompt。提供 examples 时，请尽量展示多样化的可能输入以及期望输出。
...
向模型生成请求添加额外相关 context 的技术有时称为 **retrieval-augmented generation (RAG)**。你可以用许多不同方式向 prompt 添加额外 context，例如查询向量数据库并将返回文本包含进 prompt，或使用 OpenAI 内置的 [file search tool](https://developers.openai.com/api/docs/guides/tools-file-search)，基于上传文档生成内容。
...
如需完整的当前说明，请使用 [prompt guidance](https://developers.openai.com/api/docs/guides/prompt-guidance) 指南。下面的实用提醒仍然适用。
...
如需 coding 专用的详细 guidance 和 prompt samples，请参阅我们的 [prompt guidance](https://developers.openai.com/api/docs/guides/prompt-guidance) 指南。
...
如需 frontend development 专用的详细 guidance 和 prompt samples，请参阅我们的 [prompt guidance](https://developers.openai.com/api/docs/guides/prompt-guidance) 指南。
...
如需构建 agents 专用的详细 guidance 和 prompt samples，请参阅 [prompt guidance](https://developers.openai.com/api/docs/guides/prompt-guidance) 指南。

## Prompting reasoning models

在 prompting [推理模型](https://developers.openai.com/api/docs/guides/reasoning)与 prompting GPT model 时，有一些差异需要考虑。一般来说，推理模型在只有高层 guidance 的任务上会提供更好结果。这不同于 GPT models，后者会受益于非常精确的 instructions。
...
有关使用推理模型的最佳实践的更多信息，请[参考本指南](https://developers.openai.com/api/docs/guides/reasoning-best-practices)。
