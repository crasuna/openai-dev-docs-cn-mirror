---
status: needs-review
sourceId: "4cdfef851539"
sourceChecksum: "4cdfef851539602c5ff83a709710ae97b86e813ad69bdf47c7e83a89ca04c9a3"
...
如需更详细了解 GPT-5.5 行为变化，请先阅读 [Using GPT-5.5 guide](https://developers.openai.com/api/docs/guides/latest-model)。本指南聚焦于这些行为变化带来的提示词调整。
...
对于 grounded answers，引用行为应成为提示词的一部分。定义哪些内容需要支持、什么算作足够证据，以及当证据缺失时模型应如何行动。没有证据不应自动变成事实性的“否”。如需更多细节和示例，请参阅 [citation formatting guide](https://developers.openai.com/api/docs/guides/citation-formatting)。
...
对于前端工作，请参考[示例说明](https://developers.openai.com/api/docs/guides/frontend-prompt)，了解引导 UI 质量的实用方法。这些内容涵盖产品和用户上下文、与设计系统对齐、首屏可用性、熟悉的控件、预期状态、响应式行为，以及应避免的常见生成式 UI 默认问题，例如泛化的 hero、嵌套卡片、装饰性渐变、可见说明文字和破损布局。
