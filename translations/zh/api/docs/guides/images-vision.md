...
你可以在我们的[图像生成指南](https://developers.openai.com/api/docs/guides/image-generation)中了解更多关于图像生成的内容。
...
它可以理解大多数视觉元素，包括物体、形状、颜色和纹理，但仍存在一些[限制](#limitations)。
...
关于模型如何调整图像大小，请阅读[模型尺寸处理行为](#model-sizing-behavior)部分；关于 token 成本，请阅读下方[计算成本](#calculating-costs)部分。
...
价格取决于图像尺寸和[输入保真度](https://developers.openai.com/api/docs/guides/image-generation?image-generation-model=gpt-image-1#input-fidelity)。
...
如需查看图像输入 token 的价格，请参考我们的[定价页面](https://developers.openai.com/api/docs/pricing#latest-models)。
