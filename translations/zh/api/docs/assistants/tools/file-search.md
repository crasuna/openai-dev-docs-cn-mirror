---
status: needs-review
sourceId: a5294400fb09
sourceChecksum: a5294400fb097c13ca7ee99e244663670be048ab280ea26deb799d0cd44fefae
sourceUrl: https://developers.openai.com/api/docs/assistants/tools/file-search
...
使用消息附件创建的 vector store 默认过期策略为最后一次活跃后 7 天过期（“活跃”定义为该 vector store 最后一次参与某个 run）。这个默认设置可帮助你管理向量存储成本。你可以随时覆盖这些过期策略。在[这里](#managing-costs-with-expiration-policies)了解更多。
...
默认情况下，`file_search` 工具使用以下设置，但你可以[配置](#customizing-file-search-settings)它们以满足自己的需求：
