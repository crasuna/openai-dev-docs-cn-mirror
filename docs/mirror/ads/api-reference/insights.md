---
title: "Insights（效果洞察）"
description: "Retrieve aggregated insights data across ad account, campaigns, ad groups, and ads."
outline: deep
---
...
::: warning 非官方本地镜像
- 官方来源：[https://developers.openai.com/ads/api-reference/insights](https://developers.openai.com/ads/api-reference/insights)
- Markdown 来源：[https://developers.openai.com/ads/api-reference/insights.md](https://developers.openai.com/ads/api-reference/insights.md)
- 抓取时间：2026-06-27T05:53:54.764Z
- Checksum：`209d29324de524f08fbe6bf43e72ca207000d5d2393382e854ccdc34f6c7969e`
:::
...
| `includes[]`                     | `zero_impression_items`、`zero_impression_products` | 可选的 zero-row 扩展。请参阅 [Includes](/mirror/ads/api-reference/insights#includes) 了解每个值何时可用。 |
...
| 参数                           | 类型       | 值形状 | 规则 |
| ------------------------------ | ---------- | ----------- | ----- |
| `time_granularity`             | `string`   | 一个 `time_granularity` 值 | 默认值为 `daily`。Bucket 行为请参阅 [Terminology](/mirror/ads/api-reference/insights#%E6%9C%AF%E8%AF%AD)。 |
...
| `fields`                       | `string[]` | 重复的 canonical field names | 投影所选字段。请参阅 [Projection](/mirror/ads/api-reference/insights#projection)。 |
| `filters`                      | `string[]` | JSON 编码的 filter object | 限制保留下来的行。请参阅 [Filters](/mirror/ads/api-reference/insights#filters)。 |
| `sort`                         | `string[]` | JSON 编码的 sort object | 在分页前对行排序。请参阅 [Sorts](/mirror/ads/api-reference/insights#sorts)。 |
| `segments`                     | `string[]` | 最多一个 `{segment}` | 添加一个额外细分维度。请参阅 [Segments](/mirror/ads/api-reference/insights#segments)。 |
| `override_segment_group_order` | `string[]` | 行实体加所请求 segment | 通过重新排序 groups 来改变 grouped metric 的含义。请参阅 [Segments](/mirror/ads/api-reference/insights#segments)。 |
| `includes`                     | `string[]` | 最多一个 include 值 | 使用受支持的 zero rows 扩展结果。请参阅 [Includes](/mirror/ads/api-reference/insights#includes)。 |
...
| `filters[].field`    | 来自 [Terminology](/mirror/ads/api-reference/insights#%E6%9C%AF%E8%AF%AD) 的一个 canonical field name | 字段必须对当前行形状有效。仅在精确 feed/item 对 filter 中使用 `product.feed_item_id`，并使用形如 `{"feed_id":"feed_1","item_id":"sku_1"}` 的 JSON-string `IN` 值。 | `campaign.id` 或 `ad.clicks` |
...
| `sort[].field`     | 来自 [Terminology](/mirror/ads/api-reference/insights#%E6%9C%AF%E8%AF%AD) 的一个 canonical sort key | 使用对当前行形状有效的 sort key。 | `ad.clicks` 或 `product.title` |
...
| Product fields       | 从 [Terminology](/mirror/ads/api-reference/insights#%E6%9C%AF%E8%AF%AD) 投影 `product.*` 字段。 |
| Product-first rows   | 先设置 `override_segment_group_order[]=product`，再设置 `override_segment_group_order[]=&lt;aggregation_level&gt;`。 |
| Zero-impression rows | 添加 `includes[]=zero_impression_products`；请求要求请参阅 [Includes](/mirror/ads/api-reference/insights#includes)。 |
...
| `includes[]`                     | `zero_impression_items`, `zero_impression_products`                                                                                                                                                                                                                                                                                                                                                 | Optional zero-row expansions. See [Includes](/mirror/ads/api-reference/insights#includes) for when each value works.                                                                                                                                                                                                                                          |
...
| ------------------------------ | ---------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `time_granularity`             | `string`   | One `time_granularity` value             | Default `daily`. See [Terminology](/mirror/ads/api-reference/insights#terminology) for bucket behavior.                                                                                                      |
...
| `fields`                       | `string[]` | Repeated canonical field names           | Project selected fields. See [Projection](/mirror/ads/api-reference/insights#projection).                                                                                                                    |
| `filters`                      | `string[]` | JSON-encoded filter objects              | Restrict which rows survive. See [Filters](/mirror/ads/api-reference/insights#filters).                                                                                                                      |
| `sort`                         | `string[]` | JSON-encoded sort objects                | Order rows before pagination. See [Sorts](/mirror/ads/api-reference/insights#sorts).                                                                                                                         |
| `segments`                     | `string[]` | At most one `{segment}`                  | Add one extra breakdown dimension. See [Segments](/mirror/ads/api-reference/insights#segments).                                                                                                              |
| `override_segment_group_order` | `string[]` | Row entity plus requested segment        | Change grouped metric meaning by reordering groups. See [Segments](/mirror/ads/api-reference/insights#segments).                                                                                             |
| `includes`                     | `string[]` | At most one include value                | Expand results with supported zero rows. See [Includes](/mirror/ads/api-reference/insights#includes).                                                                                                        |
...
| `filters[].field`    | One canonical field name from [Terminology](/mirror/ads/api-reference/insights#terminology) | The field must be valid for the current row shape. Use `product.feed_item_id` only for an exact feed/item pair filter with JSON-string `IN` values shaped like `{"feed_id":"feed_1","item_id":"sku_1"}`. | `campaign.id` or `ad.clicks`                                   |
...
| `sort[].field`     | One canonical sort key from [Terminology](/mirror/ads/api-reference/insights#terminology) | Use a sort key valid for the current row shape. | `ad.clicks` or `product.title`             |
...
| Product fields       | Project `product.*` fields from [Terminology](/mirror/ads/api-reference/insights#terminology).                                             |
| Product-first rows   | Set `override_segment_group_order[]=product`, then `override_segment_group_order[]=&lt;aggregation_level&gt;`. |
| Zero-impression rows | Add `includes[]=zero_impression_products`; see [Includes](/mirror/ads/api-reference/insights#includes) for request requirements.           |
