...
::: warning 非官方本地镜像
- 官方来源：[https://developers.openai.com/api/docs/guides/tools-tool-search](https://developers.openai.com/api/docs/guides/tools-tool-search)
- Markdown 来源：[https://developers.openai.com/api/docs/guides/tools-tool-search.md](https://developers.openai.com/api/docs/guides/tools-tool-search.md)
- 抓取时间：2026-06-27T05:54:11.183Z
- Checksum：`58486297f0ec58f43998b62f3e70b24709ce6fabb2d17e81bf3161c606fde204`
:::
...
2. 如果你使用 [functions](/mirror/api/docs/guides/function-calling#defining-functions)，请将想要延迟加载的函数标记为 `defer_loading: true`。如果你使用 [MCP servers](/mirror/api/docs/guides/tools-connectors-mcp)，请在 MCP server 工具定义上设置 `defer_loading: true`。

### 尽可能使用 namespaces

你可以将 tool search 与延迟的 [functions](/mirror/api/docs/guides/function-calling#defining-functions)、[namespaces](/mirror/api/docs/guides/function-calling#defining-namespaces) 或 [MCP servers](/mirror/api/docs/guides/tools-connectors-mcp) 一起使用，但我们建议尽可能使用 namespaces 或 MCP servers。我们的模型主要是在这些 surface 上训练来进行搜索的，并且在那里 token savings 通常更明显。
...
当你已经知道希望模型搜索的 [functions](/mirror/api/docs/guides/function-calling#defining-functions)、[namespaces](/mirror/api/docs/guides/function-calling#defining-namespaces) 或 [MCP servers](/mirror/api/docs/guides/tools-connectors-mcp) 的完整 inventory 时，hosted tool search 是最简单的路径。你先声明它们，添加 `{"type": "tool_search"}`，然后让 API 决定加载什么。
...
- 使用 [function calling](/mirror/api/docs/guides/function-calling) 定义 callable functions 和 custom tools。
- 使用 [Using tools](/mirror/api/docs/guides/tools) 了解 Responses 中更广泛的工具图景。
...
2. If you are using [functions](/mirror/api/docs/guides/function-calling#defining-functions), mark the ones you want to defer with `defer_loading: true`. If you are using [MCP servers](/mirror/api/docs/guides/tools-connectors-mcp), set `defer_loading: true` on the MCP server tool definition.

### Use namespaces where possible

You can use tool search with deferred [functions](/mirror/api/docs/guides/function-calling#defining-functions), [namespaces](/mirror/api/docs/guides/function-calling#defining-namespaces), or [MCP servers](/mirror/api/docs/guides/tools-connectors-mcp), but we recommend using namespaces or MCP servers when possible. Our models have primarily been trained to search those surfaces, and token savings are usually more material there.
...
Hosted tool search is the simplest path when you already know the full inventory of [functions](/mirror/api/docs/guides/function-calling#defining-functions), [namespaces](/mirror/api/docs/guides/function-calling#defining-namespaces), or [MCP servers](/mirror/api/docs/guides/tools-connectors-mcp) you want the model to search. You declare them up front, add `{"type": "tool_search"}`, and let the API decide what to load.
...
- Use [function calling](/mirror/api/docs/guides/function-calling) to define callable functions and custom tools.
- Use [Using tools](/mirror/api/docs/guides/tools) for the broader tool landscape across Responses.
