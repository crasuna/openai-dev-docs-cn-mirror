...
你可以在我们的 [Image generation](/mirror/api/docs/guides/image-generation) guide 中了解更多关于图像生成的内容。
...
它可以理解大多数视觉元素，包括物体、形状、颜色和纹理，即使仍存在一些[限制](/mirror/api/docs/guides/images-vision#limitations)。
...
关于模型如何调整图像大小，请阅读 [Model sizing behavior](/mirror/api/docs/guides/images-vision#model-sizing-behavior) 部分；关于 token 成本，请阅读下方 [Calculating costs](/mirror/api/docs/guides/images-vision#calculating-costs) 部分。
...
价格取决于图像尺寸和 [input fidelity](/mirror/api/docs/guides/image-generation#input-fidelity)。
...
如需查看 image input tokens 的价格，请参考我们的 [pricing page](/mirror/api/docs/pricing#latest-models)。
