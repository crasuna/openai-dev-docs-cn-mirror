...
::: warning 非官方本地镜像
- 官方来源：[https://developers.openai.com/codex/permissions](https://developers.openai.com/codex/permissions)
- Markdown 来源：[https://developers.openai.com/codex/permissions.md](https://developers.openai.com/codex/permissions.md)
- 抓取时间：2026-06-27T05:55:05.478Z
- Checksum：`61ff6a6b55bd79dfd5e2801ec3e7db26def65e478e1c1278146ef2d1a52a5588`
:::
...
Local permission profiles 支持 macOS、Linux、WSL 和 native Windows。平台特定的详情和注意事项请参阅 [Scope and enforcement](/mirror/codex/permissions#scope-and-enforcement)。

如需 Codex cloud network settings，请参阅 [Internet Access](/mirror/codex/cloud/internet-access)。
...
Enterprise administrators 可以定义 profiles，并通过 managed `requirements.toml` 限制用户可以选择哪些 profiles。一旦存在 `allowed_permission_profiles`，未列出的 profiles 都会被拒绝，包括未列出的 built-ins 和未来 Codex 版本中新增的 profiles。推荐的 managed configuration 请参阅 [Control available permission profiles](/mirror/codex/enterprise/managed-configuration#control-available-permission-profiles)。
...
Profiles 描述 session 的 local default posture。Organization-managed requirements 仍然可以添加用户配置不应拓宽的 restrictions。请参阅 [Managed configuration](/mirror/codex/enterprise/managed-configuration)，了解 admin-enforced filesystem 和 network constraints。
...
- 在 native Windows 上，[`elevated` sandboxing](/mirror/codex/windows#windows-sandbox) 最强，因为它可以使用专用的 lower-privilege sandbox users、filesystem permission boundaries 和 firewall rules。`unelevated` sandboxing 是隔离性较弱的 fallback，其 network isolation 较弱，且无法 enforce 每个 split read/write carveout，因此不受支持的 policies 会被拒绝。当你需要 Linux sandbox model 时，请使用 WSL。
...
Windows. See [Scope and enforcement](/mirror/codex/permissions#scope-and-enforcement) for platform-specific
details and caveats.

For Codex cloud network settings, see [Internet Access](/mirror/codex/cloud/internet-access).
...
[Control available permission profiles](/mirror/codex/enterprise/managed-configuration#control-available-permission-profiles)
...
broaden. See [Managed configuration](/mirror/codex/enterprise/managed-configuration)
...
- On Linux and WSL, Codex uses [bubblewrap](https://github.com/containers/bubblewrap)
  ...
  are refused.
- On native Windows, [`elevated` sandboxing](/mirror/codex/windows#windows-sandbox)
