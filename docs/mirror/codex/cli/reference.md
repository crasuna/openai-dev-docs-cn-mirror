...
export const globalFlagOptions = [
  {
    ...
    type: "string",
    description:
      "用于启动会话的可选文本指令。省略时会启动 TUI，且不预填消息。",
  },
  {
    key: "--image, -i",
    type: "path[,path...]",
  ...
  },
  {
    ...
    description:
      "为模型生成的 shell 命令选择 sandbox policy。",
  },
  {
  ...
  },
  {
    ...
    defaultValue: "false",
    description:
      '启用实时网页搜索（把 `web_search` 设置为 `"live"`，而不是默认的 `"cached"`）。',
  },
  {
    key: "--add-dir",
    type: "path",
  ...
  },
  {
    ...
    defaultValue: "false",
    description:
      "禁用 TUI 的 alternate screen 模式（覆盖本次运行的 `tui.alternate_screen`）。",
  },
  {
    key: "--remote",
    type: "ws://host:port | wss://host:port | unix:// | unix://PATH",
    description:
      "通过 WebSocket 或 Unix socket 连接到远程 app-server endpoint。支持 `codex`、`codex resume`、`codex fork`、`codex archive`、`codex delete` 和 `codex unarchive`；其他子命令会拒绝 remote 模式。",
  },
  {
...
export const commandOverview = [
  {
    ...
    type: "stable",
    description:
      "启动终端 UI。接受上面的全局 flags，以及可选提示词或图像附件。",
  },
  {
  ...
  },
  {
    ...
    type: "stable",
    description:
      "按 session ID 或 session name 归档已保存的交互式会话。",
  },
  {
    key: "codex delete",
  ...
  },
  {
    ...
    type: "stable",
    description:
      "以非交互方式运行 Codex。别名：`codex e`。把结果流式输出到 stdout 或 JSONL，并可选择恢复之前的会话。",
  },
  {
  ...
  },
  {
    ...
    type: "stable",
    description:
      "把之前的交互式会话 fork 成一个新 thread，同时保留原始 transcript。",
  },
  {
    key: "codex sandbox",
  ...
  },
  {
    ...
    type: "stable",
    description:
      "按 session ID 或 session name 恢复已归档的交互式会话。",
  },
];
...
export const execOptions = [
  ...
  },
  {
    ...
    type: "boolean",
    defaultValue: "false",
    description: "运行时不把会话 rollout files 持久化到磁盘。",
  },
  {
  ...
  },
  {
    ...
    description:
      "按 ID 恢复 exec 会话，或添加 `--last` 继续当前工作目录中最近的会话。添加 `--all` 可考虑来自任意目录的会话。接受可选的后续提示词。",
  },
  {
    key: "-c, --config",
    type: "key=value",
...
export const forkOptions = [
  {
    ...
    type: "uuid",
    description:
      "Fork 指定会话。省略并使用 `--last` 可 fork 最近的会话。",
  },
  {
    key: "--last",
    type: "boolean",
...
export const archiveOptions = [
  {
    ...
    description:
      "要归档或恢复的已保存会话。Session IDs 优先于 session names。",
  },
  {
    key: "--remote",
    type: "ws://host:port | wss://host:port | unix:// | unix://PATH",
...
export const deleteOptions = [
  {
    ...
    type: "session ID | session name",
    description:
      "要删除的已保存会话。Session IDs 优先于 session names。",
  },
  {
    key: "--force",
    type: "boolean",
...
本页列出了每个已记录的 Codex CLI 命令和 flag。使用交互式表格按 key 或 description 搜索。每个 section 都会说明该选项是 stable 还是 experimental，并指出有风险的组合。
...
不带子命令运行 `codex` 会启动交互式终端 UI（TUI）。该 agent 接受上面的全局 flags 以及图像附件。Web search 默认使用 cached 模式；使用 `--search` 可切换到 live browsing。为了低摩擦的本地工作，请使用 `--sandbox workspace-write --ask-for-approval on-request`。
...
按 session ID 或 session name 永久删除已保存的交互式会话。只有当你想删除 transcript，而不是仅从 active session lists 中隐藏它时，才使用此命令。
...
管理存储在 `$CODEX_HOME/config.toml` 中的 feature flags。`enable` 和 `disable` 命令会持久保存改动，使其应用于未来会话。`features` 子命令不接受 `--profile`。
...
Codex 默认写入格式化输出。添加 `--json` 可接收 newline-delimited JSON events（每次状态变化一个）。可选的 `resume` 子命令让你可以继续非交互式任务。使用 `--last` 选择当前工作目录中最近的会话，或添加 `--all` 跨所有会话搜索：
...
按 ID 继续交互式会话，或恢复最近的对话。除非你传入 `--all`，否则 `codex resume` 会把 `--last` 限定到当前工作目录。它接受与 `codex` 相同的全局 flags，包括 model 和 sandbox overrides。



### `codex fork`

把之前的交互式会话 fork 成一个新 thread。默认情况下，`codex fork` 会打开 session picker；添加 `--last` 可改为 fork 最近的会话。
...
- 在 CI 中把 `--json` 与 `--output-last-message` 搭配使用，以捕获机器可读进度和最终自然语言摘要。
