...
威胁模型是扫描时用于 repository 的安全上下文。它把简明项目概览与攻击面详情结合起来，例如入口点、信任边界、身份验证假设和风险组件。更多详情请参见 [Improving the threat model](/mirror/codex/security/threat-model)。
...
可以。Codex Security 会创建初始威胁模型，你可以随着架构、风险和业务上下文变化而更新它。有关编辑工作流，请参见 [Improving the threat model](/mirror/codex/security/threat-model)。

### 使用威胁建模前需要配置扫描吗？

需要。威胁模型 guidance 与你如何扫描以及扫描什么绑定，因此你需要先配置 repository。参见 [Codex Security setup](/mirror/codex/security/setup)。
