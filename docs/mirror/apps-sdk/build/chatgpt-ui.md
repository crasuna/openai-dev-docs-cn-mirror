...
如需 high-level overview 以及从 Apps SDK APIs 映射的指南，请参阅 [MCP Apps compatibility in ChatGPT](/mirror/apps-sdk/mcp-apps-in-chatgpt)。
...
有关使用 `postMessage` 实现最小 request/response 的示例，请参阅 quickstart：[Quickstart](/mirror/apps-sdk/quickstart#build-a-web-component)。
...
完整 API reference 请参阅 [Apps SDK Reference](/mirror/apps-sdk/reference#windowopenai-component-bridge)。
...
[examples page](/mirror/apps-sdk/build/examples#pizzaz-list-source) 包含示例 apps，例如列出披萨餐厅的 "Pizza list" app。

### 探索 Pizzaz component gallery

[Apps SDK examples](/mirror/apps-sdk/build/examples) 包含示例 components。在塑造自己的 UI 时，可以把它们当作蓝图：
...
有关如何在 MCP server response 中嵌入 component，请参阅 [Set up your server docs](/mirror/apps-sdk/build/mcp-server)。
