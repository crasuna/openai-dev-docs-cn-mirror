...
当你已经在 Developer Mode 中构建并 [测试你的应用](/mirror/apps-sdk/deploy/testing) 后，可以通过当前基于 dashboard 的审核流程提交它。该流程仍是目前公开分发的路径。当你发布获批应用时，OpenAI 会为 Codex 分发创建 plugin。
...
自助 plugin 发布即将推出。请参阅 [构建 plugin 指南](/mirror/codex/plugins/build)，了解打包模型和本地测试工作流。

_提交前，请阅读并确保你的应用符合我们的 [应用提交指南](/mirror/apps-sdk/app-submission-guidelines)。_
...
- 你没有使用本地或测试 endpoint
- 你定义了 [content security policy (CSP)](/mirror/apps-sdk/build/mcp-server#content-security-policy-csp)，以允许你获取内容的确切域名（出于安全原因，这是提交应用的必需项）
